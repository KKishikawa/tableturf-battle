...
import saveDeckButtonHTML from "./saveDeckBtn.template.html";
import clearButtonHTML from "./clearBtn.template.html";
...
cardListManager.wrapper.classList.add(
...
const defaultShowDeckCountFunc =
  deckEditManager.showCount.bind(deckEditManager);
/* デッキ情報表示メソッドをオーバーライドする */
deckEditManager.showCount = () => {
  defaultShowDeckCountFunc();
  tabBtns[1].innerHTML =
    deckEditManager.wrapper.querySelector(".table-title-text")!.innerHTML;
};
...

{
  // デッキ保存
  const btnWrapper =
    deckEditManager.wrapper.getElementsByClassName("action-wrapper")[0];
