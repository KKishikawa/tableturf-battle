...
import gridTemplate from './grid.svg.mustache';

/** カードの塗り範囲をグリッド表現 */
export function createCardGrid(g: string | null | undefined, sg: string | null | undefined) {
...
function convertNumGrit(num: number) {
  return { x: (num % 8) * 12 + 0.5, y: ~~(num / 8) * 12 + 0.5 };
}
