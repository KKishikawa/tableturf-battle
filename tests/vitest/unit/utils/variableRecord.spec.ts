...
import * as variableRecord from '@/utils/variableRecord';
{
  ...
  describe('readVariableRecord', function () {
    it.each(['', null, 'qq@w', '1x+t', '0fff(6z133', undefined])('invalid string (%p) ', function (val) {
...
describe('calcVariableRecordLen', function () {
  const fixRecordInfo = [[['dkEN0-', 4]], [['1あれ', 0]], [[null, 0]], [[undefined, 0]]] as const;
  it.each(fixRecordInfo)('string %p', function ([val, len]) {
    expect(variableRecord.calcVariableRecordLen(val)).toEqual(len);
  });
});
describe('calcFixRecordLen', function () {
  const fixRecordInfo = [[['dkEN0-', 6]], [['1あれ', 0]], [[null, 0]], [[undefined, 0]]] as const;
  it.each(fixRecordInfo)('string %p', function ([val, len]) {
    expect(variableRecord.calcFixRecordLen(val)).toEqual(len);
  });
});
describe('write => read', function () {
  it.each([[[166, 190, 200, 355, 400]], [[10, 34, 0, 3, 4, 4, 5, 6, 6]], [[1, 2, 3, 64, 55, 10]]])(
    'number array %p',
    function (val) {
...
describe('write => read (fixed)', function () {
  it.each([[[]], [[8, 10, 21, 3, 50]], [[10, 34, 0, 3, 4, 4, 5, 6, 6]], [[1, 2, 3, 63, 55, 10]]])(
    'number array %p',
    function (val) {
      expect(variableRecord.readFixRecord(variableRecord.writeFixRecord(val))).toEqual(val);
    },
  );
});
